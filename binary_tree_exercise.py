class BST:
    ...
    def nth_largest(self, n, node=None):
        if node is None:
            node = self.root
        result = self._nth_largest(n, node)
        if result is not None:
            self.count = 0
            return result.data
        else:
    ...
    def _nth_largest(self, n, node):
        if node is None:
            return None
        left = self._nth_largest(n, node.right)
        if left is not None:
            return left
        self.count += 1
        if self.count == n:
            return node
        return self._nth_largest(n, node.left)
